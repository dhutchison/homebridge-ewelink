...
import { BaseDevice, DeviceState } from './commonApi';
...
export class LanClient {
    ...
    * for the device. 
    */
   private readonly deviceHostMap: Cache<HostDetails>;

   /**
    * Event emmitter for sending events 
    */
    ...
    * 
    */
    private readonly deviceStatusMap: Cache<DeviceState>;

    /**
     * Map of deviceId to their corresponding deviceKey.
     * This is required to allow decryption of DNS data to occur. 
     */
    private readonly deviceKeyMap: Map<string, string>;

    /** 
     * DNS timeout used when getting the status for a device (in milliseconds) 
     */
    private readonly dnsTimeout = 3000;

    /** 
     * Configuration setting which dictates if we will log DNS messages or not.
     */
    private readonly logDnsResponses: boolean;
    ...
    constructor(
        ...
        if ('object' === typeof config.experimentalLanClient) {
            /* Is a configuration object, get some fields */
            this.logDnsResponses = (config.experimentalLanClient.logDnsResponses === true);
        } else {
            /* Default values */
            this.logDnsResponses = false;
        }
    ...
    _start() {
        ...
        this.sendDnsQuery();
        setInterval(() => this.sendDnsQuery(), (120 * 1000));
    }
     ...
     */
    private getCachedStatusForDevice(deviceId: string): LanDevice | undefined {
        
        let status: LanDevice | undefined;
     ...
     */
    addDeviceKey(deviceId: string, deviceKey: string) {
     ...
     */
    addDeviceStateListener(funct: (...args: any[]) => void) {
    ...
    processDnsResponse(value, notify = false) {
        ...
        if (value.type === 'TXT') {
            ...
            const stateData = this.extractDataFromDnsService(dataObject, 
                ...
                this.deviceKeyMap.get(deviceId));
            ...
            this.log.debug('Host details for %s is: %o', deviceId, hostDetails);

            /* Add the host details to a cache */
            this.deviceHostMap.set(deviceId, hostDetails);
     ...
     */
    getDeviceStatus(deviceId: string): Promise<LanDevice> {

        return new Promise<LanDevice>((resolve, reject) => {

            /* Check if we have the details we need in the cache */
            let cachedStatus = this.getCachedStatusForDevice(deviceId);

            if (cachedStatus) {
                /* Exists in the cache, return */
                this.log.debug('Returning cached result for device %s: %o', deviceId, cachedStatus);
                ...
                const timerId = setTimeout(() => {
                    ...
                    cachedStatus = this.getCachedStatusForDevice(deviceId);
                    if (cachedStatus) {
                    ...
                    if (response.answers) {
                        response.answers
                            ...
                            .forEach(value => {
                                ...
                                if (status && status.host && status.params) {
                                    /* Resolve the promise */

                                    let lanDeviceStatus: LanDevice = {
                                        deviceid: deviceId,
                                        host: status.host,
                                        params: status.params
                                    }

                                    resolve(lanDeviceStatus);
                                    /* Clean up */
                                    mdnsClient.destroy();
    ...
    */
    async doApiCall(deviceId: string, path: string, data) {
        ...
        if (localDeviceHost === undefined) {
            throw new Error('Could not find host for device id ' + deviceId);
        }
        let connectionHost = 'http://' + localDeviceHost.host.host;
        if (localDeviceHost.host.port) {
            connectionHost += ':' + localDeviceHost.host.port;
        }
    ...
    updateDeviceStatus(accessory, statusParams) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!path) {
                ...
                /* Do a lan API call */
                this.doApiCall(accessory.context.deviceId, path, statusParams)
                    .then(result => {
...
/**
 * Interface containing the host & port to connect to for a device. 
 */
interface HostConnectionDetails {
    /**
     * The host name or IP address
     */
    host: string;
    /**
     * The port to connect to the device on
     */
    port: number;
}

/**
 * Interface containing the details of a Lan Client Host device. 
 * 
 * As well as the connection details, this contains the record from DNS it
 * was created from. 
 */
interface HostDetails {

    /**
     * The processed host details
     */
    host: HostConnectionDetails;
    /**
     * The raw SRV record the host details came from
     */
    srv: any;
}

/**
 * Interface including the additional device details we have for a Lan client
 * device. 
 */
interface LanDevice extends BaseDevice {
    /**
     * The connection details for the host. 
     */
    host: HostConnectionDetails;
}

/**
 * Interface defining an object with optional fields which is used as we build up
 * the device details. 
 */
interface LanDeviceBuilder {
    host?: HostConnectionDetails;
    params?: DeviceState;
}