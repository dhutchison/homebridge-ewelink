...
import { BaseDevice, DeviceState, PlugData, StripData } from './commonApi';

// import { } from 'request-json';
...
export class EwelinkApi {
    ...
    /** 
     * Event emmitter for login events 
     */
    private readonly loginEventEmitter = new EventEmitter();
    ...
    /**
     * Authentication token retrieved from the login response.
     */
    private authenticationToken: undefined | string;
     ...
     */
    constructor(
        private readonly log: Logger, config) {
        ...
        if (!config ||
            ...
            throw new Error('Initialization skipped. Missing configuration data.');
        }
                ...
                webClient.post('/api/user/login', data)
                    .then(result => {
                        ...
                        if (result.body.error && result.body.error == 301 && result.body.region) {
                            ...
                            if (idx == -1) {
                                this.log.error('Received new region [%s]. However we cannot construct the new API host url.', result.body.region);
                                ...
                                if (this.config.apiHost != newApiHost) {
                                    this.log.info('Received new region [%s], updating API host to [%s].', result.body.region, newApiHost);
...
/**
 * Interface defining settings a device can have. 
 */
interface Settings {
    opsNotify: boolean;
    opsHistory: boolean;
