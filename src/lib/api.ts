...
import { EwelinkApi, Device }  from './ewelinkApi';
import { LanClient } from './sonoffLanModeApi';
import { WebSocketClient } from './webSocketClient';

export { BaseDevice, DeviceState } from './commonApi';
export { Device } from './ewelinkApi';
...
export class ApiClient {
     ...
     * The HTTP API client
     */
    private readonly ewelinkApiClient: EwelinkApi;

    /**
     * The WebSocket API client.
     */
    private wsc: WebSocketClient | undefined;

    /**
     * The Lan API client.
     */
    private readonly lanClient: LanClient | undefined;
    ...
    init() {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.config['countryCode']) {
                ...
                this.ewelinkApiClient.getRegion(this.config['countryCode'])
                    .then(region => {
                        ...
                        if (idx == -1) {
                            this.log.warn('Received region [%s]. However we cannot construct the new API host url.', region);
     ...
     */
    close() {
        /* If the lan client is configured, close it */
        if (this.lanClient) {
     ...
     * @param {*} accessory 
     */
    async getDeviceStatus(accessory): Promise<BaseDevice> {
        ...
        let deviceStatus: BaseDevice | undefined;
        ...
        if (deviceStatus === undefined) {
            if (this.config.experimentalWebSocketClient) {
                ...
                if (deviceStatus.online !== true) {
                    accessory.reachable = false;
                    this.log.warn("Device [%s] was reported to be offline by the API", accessory.displayName);
